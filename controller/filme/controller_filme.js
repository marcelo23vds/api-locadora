...
//import das controllers das tabelas intermediarias
        ...
        if (resultFilmes) {
            if (resultFilmes.length > 0) {
                ...
                for (filme of resultFilmes) {
                    ...
                    if (resultGeneros.status_code == 200) filme.genero = resultGeneros.items.filmes_generos

                    let resultAtores = await controllerFilmeAtor.listarAtoresIdFilme(filme.id)
                    if (resultAtores.status_code == 200) filme.ator = resultAtores.items.filmes_atores

                    let resultDiretores = await controllerFilmeDiretor.listarDiretoresIdFilme(filme.id)
                    if (resultDiretores.status_code == 200) filme.diretor = resultDiretores.items.filmes_diretores
            ...
            if (!validar) {
                ...
                if (resultFilmes) {
                    ...
                    if (lastID) {
                        ...
                        // ATORES
                        if (filme.ator && filme.ator.length > 0) {
                            for (let ator of filme.ator) {
                                let filmeAtor = { id_filme: lastID, id_ator: ator.id_ator }
                                await controllerFilmeAtor.inserirFilmeAtor(filmeAtor, contentType)
                            }
                        }
                        ...
                        if (filme.diretor && filme.diretor.length > 0) {
                            for (let diretor of filme.diretor) {
                                let filmeDiretor = { id_filme: lastID, id_diretor: diretor.id_diretor }
                                await controllerFilmeDiretor.inserirFilmeDiretor(filmeDiretor, contentType)
                            }
                        }
            ...
            if (!validar) {
                ...
                if (resultFilmes) {
                    ...
                    if (lastID) {
                        ...
                        await controllerFilmeDiretor.excluirFilmeDiretor(id)
                        ...
                        if (filme.ator && filme.ator.length > 0) {
                            for (let ator of filme.ator) {
                                let filmeAtor = { id_filme: id, id_ator: ator.id_ator }
                                await controllerFilmeAtor.inserirFilmeAtor(filmeAtor, contentType)
                            }
                        }
                        ...
                        if (filme.diretor && filme.diretor.length > 0) {
                            for (let diretor of filme.diretor) {
                                let filmeDiretor = { id_filme: id, id_diretor: diretor.id_diretor }
                                await controllerFilmeDiretor.inserirFilmeDiretor(filmeDiretor, contentType)
                            }
                        }
                        ...
                        let resultAtores = await controllerFilmeAtor.listarAtoresIdFilme(id)
                        filme.ator = resultAtores.status_code == 200 ? resultAtores.items.filmes_atores : []

                        let resultDiretores = await controllerFilmeDiretor.listarDiretoresIdFilme(id)
                        filme.diretor = resultDiretores.status_code == 200 ? resultDiretores.items.filmes_diretores : []
